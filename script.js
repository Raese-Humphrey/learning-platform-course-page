...
function openQuizPopup(weekNumber) {
  ...
  // NEW — required for status button + auto-complete
  autoCompleteWeek(weekNumber - 1);
  updateWeekStatus();
...
/* ============================================================
   HELPER - CHECK QUIZ COMPLITION
============================================================ */
function checkQuizCompleted(weekNumber) {
  const weekQuiz = quizState[weekNumber];
  if (!weekQuiz) return false;

  // Every question in every day must be correct
  return Object.values(weekQuiz).every(dayObj =>
    Object.values(dayObj).every(q => q.correct === true)
  );
}
...
============================================================ */
function autoCompleteWeek(weekIndex) {
  const allTasks = checkAllTasksCompleted(weekIndex + 1);
  const quizDone = checkQuizCompleted(weekIndex + 1);

  const isComplete = allTasks && quizDone;

  completionState[weekIndex] = isComplete;
  ...
  const cards = document.querySelectorAll(".week-card");
  if (cards[weekIndex]) {
    cards[weekIndex].classList.toggle("completed", isComplete);
  }
}
...
function updateWeekStatus() {
  const btn = document.getElementById("week-status-btn");

  const totalWeeks = completionState.length;
  const completedWeeks = completionState.filter(v => v).length;

  if (completedWeeks === 0) {
    btn.textContent = "Not Started";
    btn.className = "status-btn status-not-started";
  }
  else if (completedWeeks < totalWeeks) {
    btn.textContent = "Ongoing";
    btn.className = "status-btn status-ongoing";
  }
  else {
    btn.textContent = "Completed";
    btn.className = "status-btn status-completed";
  }
}
